...
from torch.utils.data import DataLoader, Dataset, RandomSampler, SequentialSampler  ### 追加2024/7/11
from torch.nn.utils.rnn import pad_sequence ### 追加2024/7/11
...
"""
### 書き換え2024/7/11
class OrderedIterator(DataLoader):
    def __init__(self, dataset, batch_size, sort_key=None, batch_size_fn=None, train=True, shuffle=True):
        ...
        sampler = RandomSampler(dataset) if train and shuffle else SequentialSampler(dataset)
        
        super().__init__(dataset, batch_size=batch_size, collate_fn=self.collate_fn, sampler=sampler)
    ...
    def create_batches(self):
        if self.train:
            # For training, we shuffle the data and then sort within batches
            sampler = RandomSampler(self.dataset) if self.shuffle else SequentialSampler(self.dataset)
            batches = []
            for batch in DataLoader(self.dataset, batch_size=self.batch_size, sampler=sampler):
                batches.append(sorted(batch, key=self.sort_key))
            self.batches = batches
        else:
            # For validation/testing, we just sort the entire data once
            batches = []
