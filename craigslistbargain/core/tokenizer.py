...
def tokenize(utterance, lowercase=True):
    ...
    if lowercase:
        utterance = utterance.lower() # 文章全体を小文字に変換
    # NLTKは "xx.." をトークナイズしないので, ドットを "..." に正規化する.
    utterance = re.sub(r'\.{2,}', '...', utterance)
    # 変な文字は削除
    utterance = re.sub(r'\\|>|/', ' ', utterance)
    tokens = word_tokenize(utterance) # 文をトークナイズ("It is $5." → 'it', 'is', '$', '5', '.')
    #tokens = stick_marker_sign(tokens)
    tokens = stick_dollar_sign(tokens) # $マークをくっつける('it', 'is', '$', '5', '.' → 'it', 'is', '$5', '.')
    return tokens
