...
class Utterance(object):
    ...
    def log(self, sent_number):
        ...
        if self.gold_sent is not None:
            ...
            output += u'GOLD: {}\n'.format(tgt_sent)
            # 可能な限り最高のスコアであるgold scoreは常に0である
...
class UtteranceBuilder(object):
    ...
    def build_target_tokens(self, predictions, kb=None):
        # 自然言語に直すoutput_dataのprediction属性のテンソルには([[[10,13,9,10]]])のようにカテゴリー, インテント, 価格, 終了記号の4つしか数字が入っていない
        tokens = []
        for pred in predictions:
            token = self.vocab.to_word(pred)
            if token == markers.EOS: # 終了のマーク</s>が来たら変換終了
                break
            tokens.append(token)
        # この時点でtokensには<electronics>のようなカテゴリーとインテントしか入っていない
        return tokens
    ...
    def from_batch(self, translation_batch):
        ...
        # batcherがすでに全てをソートしているため, ソートする必要はない
